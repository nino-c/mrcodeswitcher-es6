...
#from srv.applications.users.serializers import UserSerializerExtended
from .models import *
import math
import json
...
class UserSerializerExtended(serializers.ModelSerializer):
    ...
    def get_avatar(self, obj):
        try:
            image = obj.profile.picture.url
        except ValueError:
            return ''

        tokens = image.path.split('/')
...
class InstanceMixin(serializers.ModelSerializer):
    ...
    instantiator = UserSerializerExtended(required=False, read_only=True)
...
class AppSerializerNoInstances(serializers.ModelSerializer):
    category = CategoryField()
    owner = UserSerializerExtended(read_only=True)
    category_id = serializers.SerializerMethodField(read_only=True)
