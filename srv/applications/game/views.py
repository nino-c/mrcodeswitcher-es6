...
def getAngularFiles():
    ...
    #angular_appdir = join(STATIC_ROOT, "AaaE/js")

    angular_src = join(ANGULAR_APP_DIR, 'app/src')
    modules = filter(lambda mod: isdir(join(ANGULAR_APP_DIR, 'app/src', mod)), listdir(angular_src))

    angular_includes = reduce(
        lambda a,b: a+b, map(
            lambda module: sorted(
                filter(
                    lambda file: file.endswith('.js'),
                        map(lambda f: join("/app/src/", module, f),
                            listdir(join(angular_src, module)))
                    )), modules)
        )
    return angular_includes

def gameindex(request):
    #print getAngularFiles()
...
def snapshot(request, format=None):
    if request.method == 'POST':
        ...
        # write to /tmp
        file = open(join("/tmp", imagename), 'w')
        file.write(imageBIN)
        file.close()

        # save new snapshot object
        file =  open(join("/tmp", imagename), 'r')
...
def saveGIF(request):
    ...
    if request.method == 'POST':
        ...
        # write to /tmp
        file = open(join(MEDIA_ROOT, "animated_gif", imagename), 'w')
        file.write(imageBIN)
        file.close()
